...
test('API uses POST plus nonce-correlated JSONP while pairing remains single-attempt',async()=>{const api=await text('js/api.js'),auth=await text('js/auth.js');assert.match(api,/attempt<3/);assert.match(api,/postOnce=payload=>request/);assert.match(auth,/postPair\(\{action:'pairDevice'/);assert.match(api,/form\.method='POST'/);assert.match(api,/transport=jsonp&requestId=/);assert.match(api,/result\?\.pending/);assert.doesNotMatch(api,/pairingCode.*URLSearchParams/)});
