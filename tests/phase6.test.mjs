...
test('pair transport interruption requires a deliberate retry and preserves entered code',async()=>{const app=await text('js/app.js');assert.match(app,/Tap Retry Pairing once/);assert.match(app,/dataset\.retryPairing='true'/);assert.match(app,/pairButton\.dataset\.retryPairing\?'Retry Pairing'/);assert.doesNotMatch(app,/NETWORK_ERROR[\s\S]{0,160}pairForm\.requestSubmit|NETWORK_ERROR[\s\S]{0,160}\.click\(/)});
...
test('proxy fixes the upstream target, handles Apps Script redirects explicitly, restricts CORS, and never logs bodies',async()=>{const worker=await text('worker/src/index.js'),config=await text('wrangler.jsonc');assert.match(worker,/env\.APPS_SCRIPT_URL/);assert.match(worker,/redirect: 'manual'/);assert.match(worker,/script\.googleusercontent\.com/);assert.match(worker,/upstream_extra_redirect_rejected/);assert.match(worker,/ALLOWED_ORIGIN = 'https:\/\/najibyazbeck36-boop\.github\.io'/);assert.match(worker,/MAX_BODY_BYTES/);assert.doesNotMatch(worker,/console\.|script\.google\.com\/macros/);assert.doesNotMatch(config,/APPS_SCRIPT_URL|script\.google\.com/)});
