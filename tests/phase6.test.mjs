...
function harness(seed={},responses={}){
  ...
  const service=createPairingService({
    ...
    postPair:async p=>{calls.pair.push(p);if(responses.pairError)throw responses.pairError;return{deviceToken:'runtime-token'}},
...
test('API retries transient calls but pairing remains single-attempt',async()=>{const api=await text('js/api.js'),auth=await text('js/auth.js');assert.match(api,/attempt<3/);assert.match(api,/postOnce=payload=>request/);assert.match(auth,/postPair\(\{action:'pairDevice'/);assert.match(api,/cache:'no-store'/)});
...
test('INVALID_DEVICE bootstrap clears only token and marks deliberate re-pair',async()=>{const h=harness({device_id:'stable-id',device_token:'stale-token',cached_marker:4,outbox_marker:3},{bootstrapError:new ApiError('rejected','INVALID_DEVICE')});await assert.rejects(h.service.pair('Phone',''),{code:'INVALID_DEVICE'});assert.equal(h.meta.has('device_token'),false);assert.equal(h.meta.get('device_id'),'stable-id');assert.equal(h.meta.get('cached_marker'),4);assert.equal(h.meta.get('outbox_marker'),3);assert.equal(h.meta.get('pairing_requires_new_device_id'),true);assert.equal(h.calls.pair.length,0)});
...
test('pairing UI exposes progress and prevents duplicate taps',async()=>{const app=await text('js/app.js');assert.match(app,/Pairing securely/);assert.match(app,/pairButton\.disabled=true/);assert.match(app,/if\(pairingInFlight\)return/);assert.match(app,/continuing\?'Loading…':'Pairing…'/)});
