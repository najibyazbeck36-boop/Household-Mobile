...
test('API uses an origin-and-nonce-validated iframe bridge while pairing remains single-attempt',async()=>{const api=await text('js/api.js'),auth=await text('js/auth.js');assert.match(api,/attempt<3/);assert.match(api,/postOnce=payload=>request/);assert.match(auth,/postPair\(\{action:'pairDevice'/);assert.match(api,/form\.method='POST'/);assert.match(api,/trustedBridgeOrigin\(event\.origin\)/);assert.match(api,/event\.data\?\.requestId!==requestId/);assert.doesNotMatch(api,/pairingCode.*URLSearchParams/)});
